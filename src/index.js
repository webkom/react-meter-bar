...
export default class Bar extends Component {
	...
	renderLabel() {
		...
		return this.props.labels.map((label, i) => (
			<span style={{ left: `${dist * i}%`, ...style }} className='label' key={label}>
				{label}
			</span>
		));
	}
	...
	render() {
		const { progress, barColor, label } = this.props;
		const style = {
			width: `${progress}%`,
		...
		return (
			...
			>
				<div className='progress-bar__label-holder'>{this.renderLabel()}</div>
